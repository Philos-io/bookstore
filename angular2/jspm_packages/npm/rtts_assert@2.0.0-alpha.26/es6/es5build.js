...
function run(config) {
  ...
  return gulp.src(src, {cwd: config.src}).pipe(rename(function(file) {
    file.extname = file.extname.replace('.es6', '.js');
  })).pipe(sourcemaps.init()).pipe(through2.obj(function(file, encoding, done) {
    var self = this;
    ...
    sourceMaps: true
  })).pipe(sourcemaps.write('.')).pipe(gulp.dest(config.dest));
}
;
